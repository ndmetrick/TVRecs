...
import { addToWatchProviders, getAPIKey } from '../../redux/actions';
...
function StreamingAndPurchase(props) {
		...
		const getWatchProviders = async (showId) => {
			try {
				...
				if (showInfo) {
					...
					props.addToWatchProviders({ tmdbId: props.showId, info });
				}
			} catch (err) {
				console.log(err);
			}
	...
	console.log(
		'watch providers in streaming and purchase',
		props.watchProviders,
	);
