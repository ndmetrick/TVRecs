...
const AddShow = (props) => {
	...
	const addShow = (
		...
		const hasShow = props.watchShows.find(
			(watchShow) => tmdbId == watchShow.show.tmdbId,
		)
			? 'Watch'
			: props.userShows.find((userShow) => tmdbId == userShow.show.tmdbId)
				? 'Recs'
				: props.seenShows.find((seenShow) => tmdbId == seenShow.show.tmdbId)
					? 'Filter Out'
					: null;
